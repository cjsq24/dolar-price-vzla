...
import { getHoursDiff } from "../../utils/functions";
...
interface IController {
  ...
  getHistoryPrice: (req: Request, res: Response) => void;
}

const DolarPriceController: IController = {
  getActualPrice: (req: Request, res: Response): void => {
    Cont.tryCatch(res, async () => {
      const result: any = await DolarPriceToday.find()
        .sort({ _id: -1 })
        .limit(2)
      ...
      /* if (result) {
        ...
        let dolarPriceToday: IDolarTodayFull = JSON.parse(
        ...
        if (dolarPriceToday && result[1]) {
          ...
          dolarPriceToday.platforms.map(({ platform_id: plat, price }, i) => {
            ...
            const fluctuation_percent = (
              (fluctuation_bs * 100) /
              newPrice
            ).toFixed(2);
            dolarPriceToday.platforms[i].fluctuation_bs =
              fluctuation_bs.toFixed(2);
            dolarPriceToday.platforms[i].fluctuation_percent =
              fluctuation_percent;
          });
        } else {
  ...
  getHistoryPrice: (req: Request, res: Response): void => {
    Cont.tryCatch(res, async () => {
      const query: any = await DolarPriceToday.find();
    });
  },
...
export const scrapingPriceAndInsert = async (
  type: "today" | "history"
): Promise<boolean> => {
  try {
    const last: any = await DolarPriceToday.findOne().sort({ _id: -1 });
    ...
    if (diffHours > 1) {
      ...
      if (resultScraping.length > 0 && platforms) {
        ...
        if (dolarPrice.length > 0) {
          const modelToSet =
            type === "today" ? DolarPriceToday : DolarPriceHistory;
          const result = await modelToSet.create({
            platforms: dolarPrice,
            created_at: moment.tz(Date.now(), "America/Caracas"),
          });
